...
# SAL added as skype uses long wnd IDs so can't notify BN_CLICK
def mouseClick(hwnd, button):
    '''Send a notify message to a control.'''
    msgs = {"left": (win32con.WM_LBUTTONDOWN, win32con.WM_LBUTTONUP),
            "middle": (win32con.WM_MBUTTONDOWN,win32con.WM_MBUTTONUP),
            "right": (win32con.WM_RBUTTONDOWN, win32con.WM_RBUTTONUP) }
    win32gui.SendMessage(hwnd,
                         msgs[button][0],
                         0,0) # should set these based on window position
    import time
    time.sleep(0.1)
    win32gui.SendMessage(hwnd,
                         msgs[button][1],
