 ...
 */ 
var BaseTemplateWidget_controller = {
    /**
    ...
    init:function() {
        BaseTemplateWidget_controller.update();
    },
    ...
    */
	...
	if (widget === undefined) {
	    var cbks = BaseTemplateWidget_controller.callbacks[event];
	} else {
	    var cbks = BaseTemplateWidget_controller.callbacks[widget + "." + event];
	}
	if (cbks === undefined) {
	    cbks = new Array();
	}
	cbks.push(callback);
	BaseTemplateWidget_controller.callbacks[event] = cbks;
    },
	...
	var cbks = BaseTemplateWidget_controller.callbacks[event.type];
	...
	var cbks = BaseTemplateWidget_controller.callbacks[event.widget + "." + event.type];
	if (cbks === undefined) return;
	for (var i = 0; i < cbks.length; i++) {
	    cbks[i](event);
	}
...
$('#home').live('pageshow',function(event) {
   BaseTemplateWidget_controller.init(); 
});
